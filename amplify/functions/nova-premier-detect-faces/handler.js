// Nova Premier v1 Face Detection Lambda Function
import { BedrockRuntimeClient, ConverseCommand } from "@aws-sdk/client-bedrock-runtime";
import { DynamoDBClient } from "@aws-sdk/client-dynamodb";
import { DynamoDBDocumentClient, UpdateCommand } from "@aws-sdk/lib-dynamodb";

// Nova Premier is available in us-east-1 region
const bedrock = new BedrockRuntimeClient({ region: "us-east-1" });
const dynamoClient = new DynamoDBClient({ region: process.env.AWS_REGION });
const docClient = DynamoDBDocumentClient.from(dynamoClient);

const COUNTER_TABLE_NAME = process.env.COUNTER_TABLE_NAME || 'ProcessedCounter';
const MODEL_ID = process.env.NOVA_MODEL_ID || 'amazon.nova-premier-v1:0';

// Nova Premier pricing (per 1K tokens)
const PRICING = {
  input: 0.002,
  output: 0.008
};
...
export const handler = async (event) => {
  console.log('Nova Premier Lambda started');
...
Example: [{"xmin":100,"ymin":200,"xmax":300,"ymax":400}]`;
    ...
    return successResponse({
      service: 'Amazon Nova Premier v1',
      faces: faces,
